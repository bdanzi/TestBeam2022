...
void decode(const char *filename,int max_Ev,__uint32_t runnum) {
	...
	for (b = 0;; b++) {
		...
		for (chn = 0; chn < 18; chn++) {
			...
			i = (ch.cn[1] - '0') * 10 + ch.cn[2] - '0'+b*4 -1;
			printf("Found timing calibration for channel #%d\n", i);
			fread(&bin_width[0][i][0], sizeof(float), 1024, f);
		}
	}
	n_boards = b;
	...
	for (n = 0; n<max_Ev ; n++) {
		...
		printf("Found event Event serial number #%d Second %d Millisecond %d\n", eh.event_serial_number,eh.second, eh.millisecond);
		fflush(stdout);
//		rec = new TTree(Form("Event%d", eh.event_serial_number), Form("Event%d", eh.event_serial_number));
		...
		for (b = 0; b < n_boards; b++) {
			...
			for (chn=0 ; chn<18 ; chn++) {
				...
				chn_index = (ch.cn[1] - '0')*10 + ch.cn[2] - '0' + 4*b -1;
				//std::cout << "Index chn" << chn_index << std::endl;
				if (ch.c[0] == 'C') {
					...
					for (i = 0; i < 1024; i++) {
						...
						waveform[b_index][chn_index][i] = (voltage[i] / 65536. + eh.range / 1000.0 - 0.5);
...
//						std::cout <<eh.range<<std::endl;
						...
						for (j = 0, time[b_index][chn_index][i] = 0; j < i; j++)
							time[b_index][chn_index][i] += bin_width[b_index][chn_index][(j + tch.trigger_cell) % 1024];
					}
				
				}// else if (ch.c[0] == 'A') {
					waveData.setX742ChannelData(chn_index, voltage);
				// 	// Read ADC data
			...
			t1 = time[b_index][0][(1024 - tch.trigger_cell) % 1024];
			for (chn = 1; chn < 18; chn++) {
				t2 = time[b_index][chn][(1024 - tch.trigger_cell) % 1024];
				dt = t1 - t2;
				for (i = 0; i < 1024; i++){
					time[b_index][chn][i] += dt;
					voltage_2[i]=(waveform[b_index][chn][i]);
//					voltage_2[i]-=0.45;
//					voltage_2[i]*=-1;
