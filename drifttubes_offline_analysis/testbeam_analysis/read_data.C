...
void read_data::Loop(Char_t *output, Int_t MidEv,Int_t eventn,  Float_t _gsample)
{
   dim = 1024;
  ...
  string name_file = outChar;
	...
	for (auto point : wd->getX742Data()) {
	  ...
	  */
	  ...
	  //if(!isTrg && Waves_signal_1[channel].max>10 * Waves_signal_1[channel].rms){ //nPtInR == Y.size - first,lastBin; search peak when max amplitude > 5 mV
	  //if(!isTrg && Waves_signal_1[channel].max> 10 * Waves_signal_1[channel].rms ){
	  if(!isTrg && Waves_signal_1[channel].max> 10 * Waves_signal_1[channel].rms ){
	    //cout <<"Is signal"<<endl;
	    ...
	    NPeak = FindPeaks(jentry,skipFstBin[isl],channel,((wave)Waves_signal_1[channel]).nPtInR(),&((wave)Waves_signal_1[channel]).Y[skipFstBin[isl]],((wave)Waves_signal_1[channel]).rms,&((wave)Waves_signal_1[channel]).deriv[skipFstBin[isl]],&((wave)Waves_signal_1[channel]).sderiv[skipFstBin[isl]],pkPos,pkHgt);
	    ...
	    //0.625*rms= 2 sigma
	    NPeak_clust = NPeak;
	    NPeak_clust = ClusterizationFindPeaks(cut_cluster,nElectrons_per_cluster,jentry,skipFstBin[isl],channel,((wave)Waves_signal_1[channel]).nPtInR(),((wave)Waves_signal_1[channel]).rms,pkPos_clust,pkHgt_clust,pkPos,pkHgt,NPeak_clust);
	    ...
	    for(int k=0;k<NPeak;k++){
	      if(k<NPeak-1){
		((hstPerCh*)HstPerCh[channel])->hTimeDifference->Fill((float)X[pkPos[k+1]+skipFstBin[isl]] - (float)X[pkPos[k]+skipFstBin[isl]]);
	      }
	    }
	    ...
	    ((hstPerCh*)HstPerCh[channel])->hNClusterFCluster->Fill((float)X[pkPos_clust[0]+skipFstBin[isl]]+0.5*timeRes,NPeak_clust);
	    ((hstPerCh*)HstPerCh[channel])->hNPeakFPeak->Fill((float)X[pkPos[0]+skipFstBin[isl]]+0.5*timeRes,NPeak);
	    
	    ((hstPerCh*)HstPerCh[channel])->hNPeaks->Fill((float)NPeak);
	    ((hstPerCh*)HstPerCh[channel])->hNPeaks_clust->Fill((float)NPeak_clust);
	    ((hstPerCh*)HstPerCh[channel])->hNPeaks_1->Fill(X[pkPos[NPeak-1]+skipFstBin[isl]]); 
	    ((hstPerCh*)HstPerCh[channel])->hTFstPeaks->Fill(X[pkPos[0]+skipFstBin[isl]]);
	    
	    for (int ipk=0; ipk < NPeak; ++ipk){
	      ((hstPerCh*)HstPerCh[channel])->hTPeaks->Fill(X[pkPos[ipk]+skipFstBin[isl]]);
	...
	//}
	  ...
	  if(!isTrg && Waves_signal_1[channel].max<=10*(Waves_signal_1[channel].rms) && channel<=nMaxCh ){
	  ...
	  if (savesignal_1 && !isTrg && channel <=nMaxCh) { 
	    ...
	    if(!uniqueCanvas && jentry<=200 && NPeak>0 && ((wave)Waves_signal_1[channel]).nnIntegInR()>0.1 && ((channel <= 10 && channel != 4 && channel != 0 && channel !=7 && counter_1cm>=4) || (channel == 0 || channel == 4 || channel == 7 || channel == 11 && counter_1p5cm>=3))){
	    ...
	    for (int ipk=0; ipk<NPeak; ipk++){
	      TMarker *tm = new  TMarker(X[pkPos[ipk]+skipFstBin[isl]]+0.5*timeRes, pkHgt[ipk], 23);
		...
		((hstPerCh*)HstPerCh[channel])->hTimeDifference_clust->Fill((float)X[pkPos_clust[ipk+1]+skipFstBin[isl]] - (float)X[pkPos_clust[ipk]+skipFstBin[isl]]);
	      }
	    }
	    ...
	    else if(!uniqueCanvas && jentry<=200 && NPeak>0 && ((wave)Waves_signal_1[channel]).nnIntegInR()>0.1 && ((channel <= 10 && channel != 4 && channel != 0 && channel !=7 && counter_1cm>=4) || (channel == 0 || channel == 4 || channel == 7 || channel == 11 && counter_1p5cm>=3))){
	      tmpCvsignal_1.back()->Write();          
	    }
	    theFile->cd("/");
