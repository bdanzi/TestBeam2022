Int_t FindPeaks(Int_t jentry, Int_t skipFstBin, Int_t channel, Int_t npt, Float_t *amplitude, Float_t rms, Float_t *fderiv, Float_t *sderiv, Int_t *pkPos, Float_t *pkHgt, Float_t timeRes, Float_t N_1, Float_t N_2, Float_t N_3, Float_t N_4, Float_t bslnTimeInterval, Int_t isChannel_1cm, Int_t isChannel_2cm) {
  ...
  Int_t nPks=0;
  Int_t difference;
  Int_t amplitude_firstPeak = 0;
  Int_t index_firstPeak = 0;
  Int_t counter_binseparation = 0;
  //cout <<"N1: " << N_1 << endl;
  //cout <<"N2: " << N_2 << endl;
  //cout <<"N3: " << N_3 << endl;
  //cout <<"N4: " << N_4 << endl;
  ...
  for (int ip=0; ip<(npt); ++ip) {
	...
	if(nPks==0){
	  ...
	  if(ip - pkPos[nPks-1] > 1){ // if separated electrons 
      if(counter_binseparation==1){ // from a group of electrons
        pkHgt[nPks] = amplitude_firstPeak;
	      pkPos[nPks] = index_firstPeak;
        nPks=nPks+1;
      }
      else{ // from separated  electrons
      counter_binseparation = 0;
	    pkHgt[nPks]= amplitude[ip];
	    pkPos[nPks]= ip;
	    nPks=nPks+1;
      }
      ...
      counter_binseparation = counter_binseparation +1;
	    pkHgt[nPks-1] = amplitude[ip];
	    pkPos[nPks-1] = ip;
      if (counter_binseparation==1)
      {amplitude_firstPeak = amplitude[ip];
      index_firstPeak = ip;
      }
	  }
