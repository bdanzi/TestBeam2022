...
void ReadSglChannel_test_NovTestBeam(){
  ...
  bool isNov2021TestBeam = true;
  bool isJuly2022TestBeam = false;
  bool isScanAngleStudy = true;
...
//  int Channel_1p5cm[] = {0,4,7,11}; // New Test Beam Channel 0 is weird, I would skip it!
  ...
  TGraphErrors* gr_efficiency_clusters_1cm = new TGraphErrors(number_of_graphs_summary);
    ...
    if(!isRatioSummaryPlots){
 // gr_efficiency_electrons_2cm->SetTitle(Form("Number of Electrons Found %s cm cell size Drift Tubes",tubes.Data(),energy_momentum.Data()));
...
//  gr_efficiency_clusters_1cm->SetTitle(Form("Number of Clusters Found 1 cm cell size Drift Tubes %s",energy_momentum.Data()));
  gr_efficiency_electrons_2cm->SetTitle(Form("Number of Electrons Found %s cm cell size Drift Tubes",tubes.Data()));
  gr_efficiency_clusters_2cm->SetTitle(Form("Number of Clusters Found %s cm cell size Drift Tubes",tubes.Data()));
  gr_efficiency_electrons_1cm->SetTitle("Number of Electrons Found 1 cm cell size Drift Tubes");
  gr_efficiency_clusters_1cm->SetTitle("Number of Clusters Found 1 cm cell size Drift Tubes");
  }
  else if(isRatioSummaryPlots){
  gr_efficiency_electrons_2cm->SetTitle(Form("Number of Electrons Found %s cm/1 cm cell size Drift Tubes",tubes.Data()));
  gr_efficiency_clusters_2cm->SetTitle(Form("Number of Clusters Found %s cm/1 cm cell size Drift Tubes",tubes.Data()));
  }
  ...
  gr_efficiency_clusters_2cm->GetYaxis()->SetTitle("Measured Average Number of Clusters");
	...
	gr_efficiency_electrons_2cm->GetYaxis()->SetTitle(Form("Average Number of Electrons in %s cm/1 cm cell size Drift Tubes",tubes.Data()));
    gr_efficiency_clusters_2cm->GetYaxis()->SetTitle(Form("Average Number of Clusters in %s cm/1 cm cell size Drift Tubes",tubes.Data()));
  }
  ...
  if(isInteractive){ // Deprecated option, to be updated
    ...
    scanf("%d",&ev);
    if (stat(Form("/lustre/home/welmetenawee/FCC_offline_analysis/drifttubes_offline_analysis_23/testbeam_analysis/Plots/%s/",fname.Data()), &st) == -1) {
      mkdir(Form("/lustre/home/welmetenawee/FCC_offline_analysis/drifttubes_offline_analysis_23/testbeam_analysis/Plots/%s/",fname.Data()), 0700);
    }
    if (stat(Form("/lustre/home/welmetenawee/FCC_offline_analysis/drifttubes_offline_analysis_23/testbeam_analysis/Plots/%s/Waves/",fname.Data()), &st) == -1) {
      mkdir(Form("/lustre/home/welmetenawee/FCC_offline_analysis/drifttubes_offline_analysis_23/testbeam_analysis/Plots/%s/Waves/",fname.Data()), 0700);
    }
  }
  else{
    ...
    //  string Runs_90_10[] = {"histosTB_run_72.root", "histosTB_run_73.root", "histosTB_run_74.root", "histosTB_run_86.root", "histosTB_run_87.root", "histosTB_run_88.root", "histosTB_run_89.root", "histosTB_run_90.root", "histosTB_run_91.root", "histosTB_run_92.root", "histosTB_run_93.root", "histosTB_run_94.root", "histosTB_run_95.root", "histosTB_run_96.root", "histosTB_run_97.root" ,"histosTB_run_98.root", "histosTB_run_99.root", "histosTB_run_100.root", "histosTB_run_101.root"};
      ...
      //   else if(isJuly2022TestBeam){//
	  /////////////////////////////remove not needed//////
	//  string Runs_85_15[] = {"histosTB_run_71.root","histosTB_run_70.root", "histosTB_run_63.root", "histosTB_run_64.root"};
  	...
  	//  string Runs_80_20[] = {"histosTB_run_40.root","histosTB_run_41.root","histosTB_run_42.root","histosTB_run_43.root"};
  	 ...
  	 // string Runs_85_15[] = {"histosTB_run_71.root","histosTB_run_65.root","histosTB_run_69.root","histosTB_run_70.root"};
	  // All Runs
  ...
  //	string Runs_alpha_60[] = {"histosTB_run_46.root"}; // 2022 July Test Beam
	...
	for(int i =0; i<ev; ++i){
	  ...
	  if (h1==0x0) { continue; }
	  //h1->SaveAs(Form("/lustre/home/welmetenawee/FCC_offline_analysis/drifttubes_offline_analysis_23/testbeam_analysis/Plots/%s/Waves/waves_ev%i.pdf",fname.Data(),i));
	...
	for(int i = 0; i<ev; ++i){
	  for(int j = 0; j<=channel; ++j){
	    ...
	    if (h1==0x0) { continue; }
	    //h1->SaveAs(Form("/lustre/home/welmetenawee/FCC_offline_analysis/drifttubes_offline_analysis_23/testbeam_analysis/Plots/%s/Waves/waves_ev%i_Ch%i.pdf",fname.Data(),i,j));
	...
	if(isChannel_1cm){
	  ...
	  if(isRuns_90_10 || (isRuns_80_20 && isJuly2022TestBeam && i>0) || (isRuns_80_20 && isNov2021TestBeam) || isRuns_85_15){
	    ...
	    rms_electrons_1cm = rms_electrons_1cm + (h4->GetRMS()/sqrt(h4->GetEntries()));
	  }
	  //h4->Fit("landau");
	  if(isLogarithm){
	    gPad->SetLogy(1);
	  ...
	  if(isRuns_90_10 || (isRuns_80_20 && isJuly2022TestBeam && i>0) || (isRuns_80_20 && isNov2021TestBeam) || isRuns_85_15){
	    ...
	    rms_clusters_1cm = rms_clusters_1cm + (h20->GetRMS()/sqrt(h20->GetEntries()));
	...
	} //end of if condition for 1 cm tubes
	...
	//Normalization commented for getting the ratio bet 1cm and 2 cm
...
//	rms_electrons_1cm = rms_electrons_1cm/expected_electrons;
	...
	if(savePlot_effsummary && ((name_file_compact == Runs_90_10[Runs_90_10_size - 1]) || (name_file_compact == Runs_80_20[Runs_80_20_size - 1]) || (name_file_compact == Runs_85_15[Runs_85_15_size - 1]))){
	  efficiency_clusters_1cm->SaveAs("/lustre/home/welmetenawee/FCC_offline_analysis/drifttubes_offline_analysis_23/testbeam_analysis/Plots/efficiency_clusters_1cm.pdf");
	  efficiency_electrons_1cm->SaveAs("/lustre/home/welmetenawee/FCC_offline_analysis/drifttubes_offline_analysis_23/testbeam_analysis/Plots/efficiency_electrons_1cm.pdf");
	  epc_summary_1cm->SaveAs("/lustre/home/welmetenawee/FCC_offline_analysis/drifttubes_offline_analysis_23/testbeam_analysis/Plots/epc_summary_1cm.pdf");
	  integral_summary_1cm->SaveAs("/lustre/home/welmetenawee/FCC_offline_analysis/drifttubes_offline_analysis_23/testbeam_analysis/Plots/integral_summary_1cm.pdf");
	  maximum_summary_1cm->SaveAs("/lustre/home/welmetenawee/FCC_offline_analysis/drifttubes_offline_analysis_23/testbeam_analysis/Plots/maximum_summary_1cm.pdf");
	  bsl_summary_1cm->SaveAs("/lustre/home/welmetenawee/FCC_offline_analysis/drifttubes_offline_analysis_23/testbeam_analysis/Plots/bsl_summary_1cm.pdf");
	  rms_summary_1cm->SaveAs("/lustre/home/welmetenawee/FCC_offline_analysis/drifttubes_offline_analysis_23/testbeam_analysis/Plots/rms_summary_1cm.pdf");
	  aveph_summary_1cm->SaveAs("/lustre/home/welmetenawee/FCC_offline_analysis/drifttubes_offline_analysis_23/testbeam_analysis/Plots/aveph_summary_1cm.pdf");
	  
	  efficiency_clusters_1cm->SaveAs("/lustre/home/welmetenawee/FCC_offline_analysis/drifttubes_offline_analysis_23/testbeam_analysis/Plots/efficiency_clusters_1cm.png");
	  efficiency_electrons_1cm->SaveAs("/lustre/home/welmetenawee/FCC_offline_analysis/drifttubes_offline_analysis_23/testbeam_analysis/Plots/efficiency_electrons_1cm.png");
	  epc_summary_1cm->SaveAs("/lustre/home/welmetenawee/FCC_offline_analysis/drifttubes_offline_analysis_23/testbeam_analysis/Plots/epc_summary_1cm.png");
	  integral_summary_1cm->SaveAs("/lustre/home/welmetenawee/FCC_offline_analysis/drifttubes_offline_analysis_23/testbeam_analysis/Plots/integral_summary_1cm.png");
	  maximum_summary_1cm->SaveAs("/lustre/home/welmetenawee/FCC_offline_analysis/drifttubes_offline_analysis_23/testbeam_analysis/Plots/maximum_summary_1cm.png");
	  bsl_summary_1cm->SaveAs("/lustre/home/welmetenawee/FCC_offline_analysis/drifttubes_offline_analysis_23/testbeam_analysis/Plots/bsl_summary_1cm.png");
	  rms_summary_1cm->SaveAs("/lustre/home/welmetenawee/FCC_offline_analysis/drifttubes_offline_analysis_23/testbeam_analysis/Plots/rms_summary_1cm.png");
	  aveph_summary_1cm->SaveAs("/lustre/home/welmetenawee/FCC_offline_analysis/drifttubes_offline_analysis_23/testbeam_analysis/Plots/aveph_summary_1cm.png");
	  
	}
      }   //end of filling histos for 1 cm loop
  ...
  /////////////////////////////////////////////////////////////////////////////////////////////    
	...
	// Save pdf //
	//////////////
	timediff_clust_1cm->SaveAs(Form("/lustre/home/welmetenawee/FCC_offline_analysis/drifttubes_offline_analysis_23/testbeam_analysis/Plots/%s/clust_difference_1cm.pdf",fname.Data()));
	timediff_1cm->SaveAs(Form("/lustre/home/welmetenawee/FCC_offline_analysis/drifttubes_offline_analysis_23/testbeam_analysis/Plots/%s/electrons_difference_1cm.pdf",fname.Data()));
	npeaks_1cm->SaveAs(Form("/lustre/home/welmetenawee/FCC_offline_analysis/drifttubes_offline_analysis_23/testbeam_analysis/Plots/%s/nelectrons_1cm.pdf",fname.Data()));
	//tpeaks_1cm->SaveAs(Form("/lustre/home/welmetenawee/FCC_offline_analysis/drifttubes_offline_analysis_23/testbeam_analysis/Plots/%s/tpeaks_1cm.pdf",fname.Data()));
	tfpeaks_1cm->SaveAs(Form("/lustre/home/welmetenawee/FCC_offline_analysis/drifttubes_offline_analysis_23/testbeam_analysis/Plots/%s/tfpeaks_1cm.pdf",fname.Data()));
	tlpeaks_1cm->SaveAs(Form("/lustre/home/welmetenawee/FCC_offline_analysis/drifttubes_offline_analysis_23/testbeam_analysis/Plots/%s/tlpeaks_1cm.pdf",fname.Data()));
	//hnpeaks_1cm->SaveAs(Form("/lustre/home/welmetenawee/FCC_offline_analysis/drifttubes_offline_analysis_23/testbeam_analysis/Plots/%s/hnpeaks_1cm.pdf",fname.Data()));
	hncluster_1cm->SaveAs(Form("/lustre/home/welmetenawee/FCC_offline_analysis/drifttubes_offline_analysis_23/testbeam_analysis/Plots/%s/hncluster_1cm.pdf",fname.Data()));
	hpeaks_1cm->SaveAs(Form("/lustre/home/welmetenawee/FCC_offline_analysis/drifttubes_offline_analysis_23/testbeam_analysis/Plots/%s/hpeaks_1cm.pdf",fname.Data()));
	bsl_1cm->SaveAs(Form("/lustre/home/welmetenawee/FCC_offline_analysis/drifttubes_offline_analysis_23/testbeam_analysis/Plots/%s/bsl_1cm.pdf",fname.Data()));
	max_1cm->SaveAs(Form("/lustre/home/welmetenawee/FCC_offline_analysis/drifttubes_offline_analysis_23/testbeam_analysis/Plots/%s/max_1cm.pdf",fname.Data()));
	integ_1cm->SaveAs(Form("/lustre/home/welmetenawee/FCC_offline_analysis/drifttubes_offline_analysis_23/testbeam_analysis/Plots/%s/integ_1cm.pdf",fname.Data()));
	rms_1cm->SaveAs(Form("/lustre/home/welmetenawee/FCC_offline_analysis/drifttubes_offline_analysis_23/testbeam_analysis/Plots/%s/rms_1cm.pdf",fname.Data()));
	//min->SaveAs(Form("/lustre/home/welmetenawee/FCC_offline_analysis/drifttubes_offline_analysis_23/testbeam_analysis/Plots/%s/min_ch%i.pdf",fname.Data(),i));
	npeaks_clustser_1cm->SaveAs(Form("/lustre/home/welmetenawee/FCC_offline_analysis/drifttubes_offline_analysis_23/testbeam_analysis/Plots/%s/nclusters_1cm.pdf",fname.Data()));
	cluster_population_canvas_1cm->SaveAs(Form("/lustre/home/welmetenawee/FCC_offline_analysis/drifttubes_offline_analysis_23/testbeam_analysis/Plots/%s/cluster_population_1cm.pdf",fname.Data()));
	hnelectron_1cm->SaveAs(Form("/lustre/home/welmetenawee/FCC_offline_analysis/drifttubes_offline_analysis_23/testbeam_analysis/Plots/%s/hnelectron_1cm.pdf",fname.Data()));
	//////////////
	// Save png //
	//////////////
	timediff_clust_1cm->SaveAs(Form("/lustre/home/welmetenawee/FCC_offline_analysis/drifttubes_offline_analysis_23/testbeam_analysis/Plots/%s/clust_difference_1cm.png",fname.Data()));
	timediff_1cm->SaveAs(Form("/lustre/home/welmetenawee/FCC_offline_analysis/drifttubes_offline_analysis_23/testbeam_analysis/Plots/%s/electrons_difference_1cm.png",fname.Data()));
	npeaks_1cm->SaveAs(Form("/lustre/home/welmetenawee/FCC_offline_analysis/drifttubes_offline_analysis_23/testbeam_analysis/Plots/%s/nelectrons_1cm.png",fname.Data()));
	//tpeaks_1cm->SaveAs(Form("/lustre/home/welmetenawee/FCC_offline_analysis/drifttubes_offline_analysis_23/testbeam_analysis/Plots/%s/tpeaks_1cm.pdf",fname.Data()));
	tfpeaks_1cm->SaveAs(Form("/lustre/home/welmetenawee/FCC_offline_analysis/drifttubes_offline_analysis_23/testbeam_analysis/Plots/%s/tfpeaks_1cm.png",fname.Data()));
	tlpeaks_1cm->SaveAs(Form("/lustre/home/welmetenawee/FCC_offline_analysis/drifttubes_offline_analysis_23/testbeam_analysis/Plots/%s/tlpeaks_1cm.png",fname.Data()));
	//hnpeaks_1cm->SaveAs(Form("/lustre/home/welmetenawee/FCC_offline_analysis/drifttubes_offline_analysis_23/testbeam_analysis/Plots/%s/hnpeaks_1cm.pdf",fname.Data()));
	hncluster_1cm->SaveAs(Form("/lustre/home/welmetenawee/FCC_offline_analysis/drifttubes_offline_analysis_23/testbeam_analysis/Plots/%s/hncluster_1cm.png",fname.Data()));
	hpeaks_1cm->SaveAs(Form("/lustre/home/welmetenawee/FCC_offline_analysis/drifttubes_offline_analysis_23/testbeam_analysis/Plots/%s/hpeaks_1cm.png",fname.Data()));
	bsl_1cm->SaveAs(Form("/lustre/home/welmetenawee/FCC_offline_analysis/drifttubes_offline_analysis_23/testbeam_analysis/Plots/%s/bsl_1cm.png",fname.Data()));
	max_1cm->SaveAs(Form("/lustre/home/welmetenawee/FCC_offline_analysis/drifttubes_offline_analysis_23/testbeam_analysis/Plots/%s/max_1cm.png",fname.Data()));
	integ_1cm->SaveAs(Form("/lustre/home/welmetenawee/FCC_offline_analysis/drifttubes_offline_analysis_23/testbeam_analysis/Plots/%s/integ_1cm.png",fname.Data()));
	rms_1cm->SaveAs(Form("/lustre/home/welmetenawee/FCC_offline_analysis/drifttubes_offline_analysis_23/testbeam_analysis/Plots/%s/rms_1cm.png",fname.Data()));
	//min->SaveAs(Form("/lustre/home/welmetenawee/FCC_offline_analysis/drifttubes_offline_analysis_23/testbeam_analysis/Plots/%s/min_ch%i.pdf",fname.Data(),i));
	npeaks_clustser_1cm->SaveAs(Form("/lustre/home/welmetenawee/FCC_offline_analysis/drifttubes_offline_analysis_23/testbeam_analysis/Plots/%s/nclusters_1cm.png",fname.Data()));
	cluster_population_canvas_1cm->SaveAs(Form("/lustre/home/welmetenawee/FCC_offline_analysis/drifttubes_offline_analysis_23/testbeam_analysis/Plots/%s/cluster_population_1cm.png",fname.Data()));
	hnelectron_1cm->SaveAs(Form("/lustre/home/welmetenawee/FCC_offline_analysis/drifttubes_offline_analysis_23/testbeam_analysis/Plots/%s/hnelectron_1cm.png",fname.Data()));
	...
	if((isChannel_2cm && isNov2021TestBeam) || (isChannel_1p5cm && isJuly2022TestBeam)){
	  ...
	  if(isRuns_90_10 || (isRuns_80_20 && isJuly2022TestBeam && i>0) || (isRuns_80_20 && isNov2021TestBeam) || isRuns_85_15){
	    ...
	    rms_electrons_2cm = rms_electrons_2cm + (h4->GetRMS()/sqrt(h4->GetEntries()));
	  }
	  //h4->Fit("landau");
	  if(isLogarithm){
	    gPad->SetLogy(1);
	...
	//Normalization commented for getting the ratio bet 1cm and 2 cm Walaa
...
//	rms_electrons_2cm = rms_electrons_2cm/expected_electrons;
	...
	// Added by Walaa to get the ratio bet 1cm and 2cm
	
  if(isRatioSummaryPlots){
	mean_clusters_2cm = mean_clusters_2cm/mean_clusters_1cm;
	mean_electrons_2cm = mean_electrons_2cm/mean_electrons_1cm;
	rms_clusters_2cm = rms_clusters_2cm/rms_clusters_1cm;
	rms_electrons_2cm = rms_electrons_2cm/rms_electrons_1cm;
	}
	...
	// Save pdf //
	//////////////
	  efficiency_clusters_2cm->SaveAs(Form("/lustre/home/welmetenawee/FCC_offline_analysis/drifttubes_offline_analysis_23/testbeam_analysis/Plots/efficiency_clusters_%scm.pdf",tubes.Data()));
	  efficiency_electrons_2cm->SaveAs(Form("/lustre/home/welmetenawee/FCC_offline_analysis/drifttubes_offline_analysis_23/testbeam_analysis/Plots/efficiency_electrons_%scm.pdf",tubes.Data())); 
	  integral_summary_2cm->SaveAs(Form("/lustre/home/welmetenawee/FCC_offline_analysis/drifttubes_offline_analysis_23/testbeam_analysis/Plots/integral_summary_%scm.pdf",tubes.Data()));
	  maximum_summary_2cm->SaveAs(Form("/lustre/home/welmetenawee/FCC_offline_analysis/drifttubes_offline_analysis_23/testbeam_analysis/Plots/maximum_summary_%scm.pdf",tubes.Data()));
	  epc_summary_2cm->SaveAs(Form("/lustre/home/welmetenawee/FCC_offline_analysis/drifttubes_offline_analysis_23/testbeam_analysis/Plots/epc_summary_%scm.pdf",tubes.Data()));
	  bsl_summary_2cm->SaveAs(Form("/lustre/home/welmetenawee/FCC_offline_analysis/drifttubes_offline_analysis_23/testbeam_analysis/Plots/bsl_summary_%scm.pdf",tubes.Data()));
	  rms_summary_2cm->SaveAs(Form("/lustre/home/welmetenawee/FCC_offline_analysis/drifttubes_offline_analysis_23/testbeam_analysis/Plots/rms_summary_%scm.pdf",tubes.Data()));
	  aveph_summary_2cm->SaveAs(Form("/lustre/home/welmetenawee/FCC_offline_analysis/drifttubes_offline_analysis_23/testbeam_analysis/Plots/aveph_summary_%scm.pdf",tubes.Data()));
	//////////////
	// Save png //
	//////////////
	  efficiency_clusters_2cm->SaveAs(Form("/lustre/home/welmetenawee/FCC_offline_analysis/drifttubes_offline_analysis_23/testbeam_analysis/Plots/efficiency_clusters_%scm.png",tubes.Data()));
	  efficiency_electrons_2cm->SaveAs(Form("/lustre/home/welmetenawee/FCC_offline_analysis/drifttubes_offline_analysis_23/testbeam_analysis/Plots/efficiency_electrons_%scm.png",tubes.Data())); 
	  integral_summary_2cm->SaveAs(Form("/lustre/home/welmetenawee/FCC_offline_analysis/drifttubes_offline_analysis_23/testbeam_analysis/Plots/integral_summary_%scm.png",tubes.Data()));
	  maximum_summary_2cm->SaveAs(Form("/lustre/home/welmetenawee/FCC_offline_analysis/drifttubes_offline_analysis_23/testbeam_analysis/Plots/maximum_summary_%scm.png",tubes.Data()));
	  epc_summary_2cm->SaveAs(Form("/lustre/home/welmetenawee/FCC_offline_analysis/drifttubes_offline_analysis_23/testbeam_analysis/Plots/epc_summary_%scm.png",tubes.Data()));
	  bsl_summary_2cm->SaveAs(Form("/lustre/home/welmetenawee/FCC_offline_analysis/drifttubes_offline_analysis_23/testbeam_analysis/Plots/bsl_summary_%scm.png",tubes.Data()));
	  rms_summary_2cm->SaveAs(Form("/lustre/home/welmetenawee/FCC_offline_analysis/drifttubes_offline_analysis_23/testbeam_analysis/Plots/rms_summary_%scm.png",tubes.Data()));
	  aveph_summary_2cm->SaveAs(Form("/lustre/home/welmetenawee/FCC_offline_analysis/drifttubes_offline_analysis_23/testbeam_analysis/Plots/aveph_summary_%scm.png",tubes.Data()));
	...
	// Save pdf //
	//////////////
	timediff_clust_2cm->SaveAs(Form("/lustre/home/welmetenawee/FCC_offline_analysis/drifttubes_offline_analysis_23/testbeam_analysis/Plots/%s/clust_difference_%scm.pdf",fname.Data(),tubes.Data()));
	timediff_2cm->SaveAs(Form("/lustre/home/welmetenawee/FCC_offline_analysis/drifttubes_offline_analysis_23/testbeam_analysis/Plots/%s/electrons_difference_%scm.pdf",fname.Data(),tubes.Data()));
	npeaks_2cm->SaveAs(Form("/lustre/home/welmetenawee/FCC_offline_analysis/drifttubes_offline_analysis_23/testbeam_analysis/Plots/%s/nelectrons_%scm.pdf",fname.Data(),tubes.Data()));
	//tpeaks_2cm->SaveAs(Form("/lustre/home/welmetenawee/FCC_offline_analysis/drifttubes_offline_analysis_23/testbeam_analysis/Plots/%s/tpeaks_%scm.pdf",fname.Data(),tubes.Data()));
	tfpeaks_2cm->SaveAs(Form("/lustre/home/welmetenawee/FCC_offline_analysis/drifttubes_offline_analysis_23/testbeam_analysis/Plots/%s/tfpeaks_%scm.pdf",fname.Data(),tubes.Data()));
	tlpeaks_2cm->SaveAs(Form("/lustre/home/welmetenawee/FCC_offline_analysis/drifttubes_offline_analysis_23/testbeam_analysis/Plots/%s/tlpeaks_%scm.pdf",fname.Data(),tubes.Data()));
	//hnpeaks_2cm->SaveAs(Form("/lustre/home/welmetenawee/FCC_offline_analysis/drifttubes_offline_analysis_23/testbeam_analysis/Plots/%s/hnpeaks_%scm.pdf",fname.Data()));
	hncluster_2cm->SaveAs(Form("/lustre/home/welmetenawee/FCC_offline_analysis/drifttubes_offline_analysis_23/testbeam_analysis/Plots/%s/hncluster_%scm.pdf",fname.Data(),tubes.Data()));
	npeaks_clustser_2cm->SaveAs(Form("/lustre/home/welmetenawee/FCC_offline_analysis/drifttubes_offline_analysis_23/testbeam_analysis/Plots/%s/nclusters_%scm.pdf",fname.Data(),tubes.Data()));
	hpeaks_2cm->SaveAs(Form("/lustre/home/welmetenawee/FCC_offline_analysis/drifttubes_offline_analysis_23/testbeam_analysis/Plots/%s/hpeaks_%scm.pdf",fname.Data(),tubes.Data()));
	bsl_2cm->SaveAs(Form("/lustre/home/welmetenawee/FCC_offline_analysis/drifttubes_offline_analysis_23/testbeam_analysis/Plots/%s/bsl_%scm.pdf",fname.Data(),tubes.Data()));
	max_2cm->SaveAs(Form("/lustre/home/welmetenawee/FCC_offline_analysis/drifttubes_offline_analysis_23/testbeam_analysis/Plots/%s/max_%scm.pdf",fname.Data(),tubes.Data()));
	integ_2cm->SaveAs(Form("/lustre/home/welmetenawee/FCC_offline_analysis/drifttubes_offline_analysis_23/testbeam_analysis/Plots/%s/integ_%scm.pdf",fname.Data(),tubes.Data()));
	rms_2cm->SaveAs(Form("/lustre/home/welmetenawee/FCC_offline_analysis/drifttubes_offline_analysis_23/testbeam_analysis/Plots/%s/rms_%scm.pdf",fname.Data(),tubes.Data()));
	cluster_population_canvas_2cm->SaveAs(Form("/lustre/home/welmetenawee/FCC_offline_analysis/drifttubes_offline_analysis_23/testbeam_analysis/Plots/%s/cluster_population_%scm.pdf",fname.Data(),tubes.Data()));
	//min->SaveAs(Form("/lustrehome/bdanzi/offline_analysis/testbeam_analysis/Plots/%s/min_ch%i.pdf",fname.Data(),i));
	hnelectron_2cm->SaveAs(Form("/lustre/home/welmetenawee/FCC_offline_analysis/drifttubes_offline_analysis_23/testbeam_analysis/Plots/%s/hnelectron_%scm.pdf",fname.Data(),tubes.Data()));
	//////////////
	// Save png //
	//////////////
	timediff_clust_2cm->SaveAs(Form("/lustre/home/welmetenawee/FCC_offline_analysis/drifttubes_offline_analysis_23/testbeam_analysis/Plots/%s/clust_difference_%scm.png",fname.Data(),tubes.Data()));
	timediff_2cm->SaveAs(Form("/lustre/home/welmetenawee/FCC_offline_analysis/drifttubes_offline_analysis_23/testbeam_analysis/Plots/%s/electrons_difference_%scm.png",fname.Data(),tubes.Data()));
	npeaks_2cm->SaveAs(Form("/lustre/home/welmetenawee/FCC_offline_analysis/drifttubes_offline_analysis_23/testbeam_analysis/Plots/%s/nelectrons_%scm.png",fname.Data(),tubes.Data()));
	//tpeaks_2cm->SaveAs(Form("/lustre/home/welmetenawee/FCC_offline_analysis/drifttubes_offline_analysis_23/testbeam_analysis/Plots/%s/tpeaks_%scm.pdf",fname.Data(),tubes.Data()));
	tfpeaks_2cm->SaveAs(Form("/lustre/home/welmetenawee/FCC_offline_analysis/drifttubes_offline_analysis_23/testbeam_analysis/Plots/%s/tfpeaks_%scm.png",fname.Data(),tubes.Data()));
	tlpeaks_2cm->SaveAs(Form("/lustre/home/welmetenawee/FCC_offline_analysis/drifttubes_offline_analysis_23/testbeam_analysis/Plots/%s/tlpeaks_%scm.png",fname.Data(),tubes.Data()));
	//hnpeaks_2cm->SaveAs(Form("/lustre/home/welmetenawee/FCC_offline_analysis/drifttubes_offline_analysis_23/testbeam_analysis/Plots/%s/hnpeaks_%scm.pdf",fname.Data()));
	hncluster_2cm->SaveAs(Form("/lustre/home/welmetenawee/FCC_offline_analysis/drifttubes_offline_analysis_23/testbeam_analysis/Plots/%s/hncluster_%scm.png",fname.Data(),tubes.Data()));
	npeaks_clustser_2cm->SaveAs(Form("/lustre/home/welmetenawee/FCC_offline_analysis/drifttubes_offline_analysis_23/testbeam_analysis/Plots/%s/nclusters_%scm.png",fname.Data(),tubes.Data()));
	hpeaks_2cm->SaveAs(Form("/lustre/home/welmetenawee/FCC_offline_analysis/drifttubes_offline_analysis_23/testbeam_analysis/Plots/%s/hpeaks_%scm.png",fname.Data(),tubes.Data()));
	bsl_2cm->SaveAs(Form("/lustre/home/welmetenawee/FCC_offline_analysis/drifttubes_offline_analysis_23/testbeam_analysis/Plots/%s/bsl_%scm.png",fname.Data(),tubes.Data()));
	max_2cm->SaveAs(Form("/lustre/home/welmetenawee/FCC_offline_analysis/drifttubes_offline_analysis_23/testbeam_analysis/Plots/%s/max_%scm.png",fname.Data(),tubes.Data()));
	integ_2cm->SaveAs(Form("/lustre/home/welmetenawee/FCC_offline_analysis/drifttubes_offline_analysis_23/testbeam_analysis/Plots/%s/integ_%scm.png",fname.Data(),tubes.Data()));
	rms_2cm->SaveAs(Form("/lustre/home/welmetenawee/FCC_offline_analysis/drifttubes_offline_analysis_23/testbeam_analysis/Plots/%s/rms_%scm.png",fname.Data(),tubes.Data()));
	cluster_population_canvas_2cm->SaveAs(Form("/lustre/home/welmetenawee/FCC_offline_analysis/drifttubes_offline_analysis_23/testbeam_analysis/Plots/%s/cluster_population_%scm.png",fname.Data(),tubes.Data()));
	//min->SaveAs(Form("/lustrehome/bdanzi/offline_analysis/testbeam_analysis/Plots/%s/min_ch%i.pdf",fname.Data(),i));
	hnelectron_2cm->SaveAs(Form("/lustre/home/welmetenawee/FCC_offline_analysis/drifttubes_offline_analysis_23/testbeam_analysis/Plots/%s/hnelectron_%scm.png",fname.Data(),tubes.Data()));
      }
