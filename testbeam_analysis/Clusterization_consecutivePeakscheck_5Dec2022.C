Int_t ClusterizationFindPeaks(Float_t cut,Int_t *nElectrons_per_cluster,Int_t jentry, Int_t skipFstBin, Int_t channel, Int_t npt, Float_t rms, Int_t *pkPos_clust, Float_t *pkHgt_clust, Int_t *pkPos, Float_t *pkHgt, Int_t NPeak, Float_t timeRes,Int_t isChannel_1cm, Int_t isChannel_2cm) {
  ...
  for (int ip = 0; ip < nPks_constant; ++ip) {
    if(isChannel_1cm){
    cut = 0.25*sqrt(pkPos[ip]*timeRes);
  }
  else if(isChannel_2cm){
    cut = 0.2*sqrt(pkPos[ip]*timeRes);
  }
    ...
    if(NPeak_clust == 0){
      ...
      difference_ns = (pkPos[ip] - pkPos[ip-1])*timeRes;
  ...
  if (max_amplitude > 0. && index_max > 0){
	...
	if(pkHgt[ip]> max_amplitude){ 
	  if(pkHgt[ip-1] > pkHgt[ip]){
	    max_amplitude = pkHgt[ip-1]; 
	    index_max = pkPos[ip-1]; 
	  }
	  else{
	...
	pkHgt_clust[NPeak_clust-1] = pkHgt[ip];
	pkPos_clust[NPeak_clust-1] = pkPos[ip];
      }
