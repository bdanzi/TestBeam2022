Int_t FindPeaks(Int_t jentry, Int_t skipFstBin, Int_t channel, Int_t npt, Float_t *amplitude, Float_t rms, Float_t *fderiv, Float_t *sderiv, Int_t *pkPos, Float_t *pkHgt, Float_t timeRes, Float_t N_1, Float_t N_2, Float_t N_3, Float_t N_4, Float_t bslnTimeInterval, Int_t isChannel_1cm, Int_t isChannel_2cm,Int_t isChannel_1p5cm) {
  ...
  Float_t sigd1 = (float)((sqrt(2)*rms)/(timeRes));
  Float_t sigd2 = (float)((2*rms)/(timeRes*timeRes));
  
  Int_t maxNPks = 100; //With maxNPks = 130 *** Break *** segmentation violation
  Int_t nPks=0;
  ...
  //    cout <<"Channel=   " <<channel<< "jentry= "<< jentry<<"ip= "<<ip<< "NPeak= "<< nPks<<"    pkPos[nPks-1]= "<<pkPos[nPks-1]<<"   pktime="<< pkPos[nPks-1] * timeRes<<"    pkHgt[nPks-1]= "<<pkHgt[nPks-1] <<"amplitude[j]"<<amplitude[ip]<<endl;
